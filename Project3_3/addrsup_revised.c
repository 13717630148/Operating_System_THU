...
// 
// define the structure & method of rbtree
//

#define rb_black 0
#define rb_red 1

# define rb_parent(node) (SANITIZE_PARENT_NODE((node)->u1.Parent))
# define rb_color(node) ((node)->u1.Balance)
# define rb_is_red(node) (rb_color(node))
# define rb_is_black(node) (!rb_color(node))
# define rb_set_red(node) ((node)->u1.Balance = rb_red)
# define rb_set_black(node) ((node)->u1.Balance = rb_black)

static void
rb_set_parent(
    PMMADDRESS_NODE rb, PMMADDRESS_NODE p)
{
    ...
    PMM_AVL_TABLE Table)
{
    ...
    if (parent)
    {
        ...
        Table->BalancedRoot.RightChild = right;
    ...
    PMM_AVL_TABLE Table)
{
    ...
    if (parent)
    {
        ...
        Table->BalancedRoot.RightChild = left;
...
rb_insert_color(
    ...
    PMM_AVL_TABLE Table)
{
    PMMADDRESS_NODE parent, grandparent, judge_node
    register PMMADDRESS_NODE uncle;

    rb_set_red(node);
    while ((parent = SANITIZE_PARENT_NODE(node->u1.Parent)) && rb_is_red(parent))
    {
        grandparent = rb_parent(parent);
        if (parent == grandparent->LeftChild)
        {
            uncle = grandparent->RightChild;
            judge_node = parent->RightChild;
        }
        else
        {
            uncle = grandparent->LeftChild;
            judge_node = parent->LeftChild;
        }
        ...
        rb_set_black(parent);
        rb_set_red(grandparent);
        __rb_rotate_right(grandparent, Table);
    }
...

{
    ...
    {
        PMMADDRESS_NODE old = NodeToDelete, left;
...

{
    ...
    if (SearchResult == TableEmptyTree) {
        // The root of rbtree is Balance.RightChild actually
    ...
    ASSERT((Table->NumberGenericTableElements >= MiWorstCaseFill[Table->DepthOfTree]) &&
        ...
        // 
        // Modify color to ensure the property of rbtree
        // 
